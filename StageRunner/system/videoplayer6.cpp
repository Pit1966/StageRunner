	...
	, m_stopVideoAtEventEnd(false)
{
	...
	// setNotifyInterval(20);  no more available in Qt6
...
{
	qreal level = qreal(vol) / m_videoCtrl->maxVolume();
...
bool VideoPlayer::_playVideoClip(FxClipItem *fxc, FxClipItem *old_fxc, int slotNum)
{
	...
	QMediaPlayer::Error astat = error();
	Q_UNUSED(astat)
...
void VideoPlayer::on_play_state_changed(PlaybackState state)
{
	bool restart = false;
